...
class WordDropout(WordNoising):
    ...
    def noising(self, x, lengths, fix_idx=None, dropout_prob=None, blank_idx=None):
        if dropout_prob is None:
            dropout_prob = self.default_dropout_prob
        # x: (T x B), lengths: B, fix_idx: int
        if dropout_prob == 0:
            return x, lengths
        ...
        for i in range(lengths.size(0)):
            ...
            if fix_idx is not None:
                assert num_words == x.size(0)
                fix_pos = (x[:, i] == fix_idx)
                keep[fix_pos.numpy()] = True
...
class WordShuffle(WordNoising):
    ...
    def noising(self, x, lengths, fix_idx=None, max_shuffle_distance=None):
        ...
        if fix_idx is not None:
            ...
            bias = noise.copy()
            bias[:, :] = 0
            for i in range(fix_pos.size(1)):
                fix_pos_idx = torch.where(fix_pos[:, i])[0]
                if len(fix_pos_idx) == 0:
                    continue
                for j in range(len(fix_pos_idx)):
                    bias[fix_pos_idx[j]:, i] += max_shuffle_distance + 1
            noise += bias
...
class UnsupervisedMTNoising(WordNoising):
    ...
    def noising(self, x, lengths, fix_idx=None):
        # 1. Word Shuffle
...
class NoisingDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if not isinstance(src_item, dict):
            ...
            src_tokens = src_item['src_tokens']
            fix_idx = src_item.get('fix_idx')
        ...
        with data_utils.numpy_seed(self.seed + index):
            noisy_src_tokens = self.noiser.noising(src_tokens_t, src_lengths, fix_idx)
        ...
        if not isinstance(src_item, dict):
            src_item = noisy_src_tokens[0]
        else:
            src_item['src_tokens'] = noisy_src_tokens[0]
        return src_item
