...
class BARTModel(TransformerModel):
    ...
    def load_state_dict(self, state_dict, strict=True, args=None):
        ...
        if len(models_key) == 0:
            super().load_state_dict(state_dict, strict=strict, args=args)
        else:
            langs = set()
            # for k in models_key:
            #     cur_langs = k.split('.')[1].split('-')
            #     if len(cur_langs) == 2:
            #         langs += set(cur_langs)
            state_dict_bart = {}
            src_lang = 'cnndm'
            tgt_lang = 'inshorts'
            for k in models_key:
                if k.split('.')[1] == '{}-{}'.format(src_lang, tgt_lang):
                    bart_key = '.'.join(k.split('.')[2:])
                    state_dict_bart[bart_key] = state_dict[k]
            super().load_state_dict(state_dict_bart, strict=True)
