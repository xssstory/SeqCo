...
from fairseq.tasks.back_summarization import BackSummarizationTask
from fairseq.tasks.finetune_summarization import FinetuneSummarizationTask
...
class Trainer(object):
    ...
    def save_checkpoint(self, filename, extra_state):
        ...
        if distributed_utils.is_master(self.args):  # only save one checkpoint
            ...
            if hasattr(model_to_save, 'save_pretrained'):
                save_path = filename + '.hf'
                os.makedirs(save_path, exist_ok=True)
                model_to_save.save_pretrained(save_path)
                print('*** save model to hf format {} ***'.format(save_path))
    ...
    def train_step(self, samples, dummy_batch=False, raise_oom=False):
        ...
        try:
            ...
            # if (isinstance(self.task, BackSummarizationTask) or isinstance(self.task, FinetuneSummarizationTask)) \
            #     and (self.args.momentum_contrast_loss_ratio > 0 or self.args.byol_ratio > 0):
            if hasattr(self.task, 'update_momentum_paras'):
                self.task.update_momentum_paras(self.model)
